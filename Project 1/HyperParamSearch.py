...
def hyperparameter_search(config, abs_path, epochs, cpu_cores, split_ratios, num_samples=10, max_t=100, grace_period=10):
    ...
    def train_model(config):
        ...
        dataloaders = dataloader(
            data, lookback=config["lookback"], split_ratios=split_ratios, batch_size=config["batch_size"])
        model = HydrologyLSTM(
            input_size=input_size, hidden_size=config["hidden_size"], num_layers=config["num_layers"], drop_out=config["drop_out"])
    ...
    result = tune.run(
        ...
        resources_per_trial={'cpu': cpu_cores},
        config=config,
    ...
    best_trial_summary = {"config": best_trial_config,
