...
def run_model(best_trial_config, epochs, abs_path):
    ...
    avg_loss, y_hat_test_set, y_test_set = model.predict(dataloaders['test'], ray_tune=False)
    avg_loss, y_hat_validation_set, y_validation_set = model.predict(dataloaders['validate'], ray_tune=False)
    avg_loss, y_hat_train_set, y_train_set = model.predict(dataloaders['train_not_shuffled'], ray_tune=False)
...
def run_plots(loss, train, validation, test):
    ...
    y_hat_validation_set_np = torch.cat(y_hat_validation_set, dim=0).detach().numpy()
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(y_hat_validation_set_np, label='LSTM Prediction (validation)', alpha=0.7)
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(range(len(train_loss)), train_loss, label='Training Loss', alpha=0.7)
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(range(len(validation_loss)), validation_loss, label='Validation Loss', alpha=0.7)
    plt.title('Training Loss vs Epochs')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()
...
if __name__ == "__main__":
        ...
        epochs = 100        # Number of epochs in the model training (epochs <= max_t)
        ...
        grace_period = 2   # A trial must run for at least grace_period iterations before it can be stopped early.

        cpu_cores = 4       # Number of cpu cores to be used in the hyperparameter search
         
        best_trial_config = run_hyperparameter_search(abs_path, epochs, num_samples, max_t, grace_period, cpu_cores)
        
        loss, train, validation, test = run_model(best_trial_config, epochs, abs_path)
